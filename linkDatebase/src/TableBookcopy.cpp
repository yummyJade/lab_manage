...
void writeBookcopy(int head_size, int data_size, Bookcopy bookcopy, int id) {
    ...
    */
    ...
    fwrite(temp2[0].data(), sizeof(char), 20, fp);
    fwrite(temp2[1].data(), sizeof(char), 41, fp);
    fclose(fp);
}
...
Bookcopy readBookcopy(int head_size, int data_size, int id) {
    ...
    //cout << "fine" << endl;
    fread(&temp2, sizeof(char), 20, fp);
    fread(&temp3, sizeof(char), 41, fp);
...
TableBookcopy::TableBookcopy() {
    ...
    if(fread(&this->data_size, sizeof(int), 1, fp) == 0){
        this->data_size = sizeof(int) * 4 + sizeof(char) * 61;
        flag = true;
    }
