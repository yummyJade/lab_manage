...
int DataBase::createTable(string name, const vector<string> &col_name, const vector<ll> &col_size,
                          const vector<char> &isHash, const vector<char> &isUnique) {
...
bool DataBase::getKeyLocation(string key, ll &idx, ll &selfLen, ll &preSeek, ll &lastSeek, ll &totalSeek) {
    ...
    lastSeek = (sta->table_col_pre_size[table_id][sta->table_col_num[table_id]] -
                sta->table_col_pre_size[table_id][idx + 1]) * sizeof(char);
